// ============================================================
// UI ESTUDIO DE TONOS v7.2 - COMPLETO Y CORREGIDO
// ============================================================
...
class UITonos {
    ...
    async init(core) {
        ...
        this._initDone = true;
        console.log('🎵 UI Tonos v7.2: Inicializado (completo y corregido)');
        return this;
    }
    ...
    // ============================================================
    // RENDERIZAR ESTUDIO DE TONOS - CON HISTORIAS COMPLETAS
    // ============================================================
                                    ...
                                    <div style="background:${esLeida ? 'var(--success)04' : 'var(--bg)'};border-radius:10px;padding:12px 16px;border:2px solid ${completada ? 'var(--success)' : (esLeida ? 'var(--success)' : 'var(--light)')};display:flex;flex-direction:column;gap:6px;cursor:pointer;" 
                                        ...
                                        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                                            <div style="display:flex;align-items:center;gap:10px;">
                                                <span style="font-size:18px;font-weight:700;color:${tituloColor};">${tituloIcon} ${historia.titulo || 'Historia sin título'}</span>
    ...
    async _toggleHistoriaCompletada(idx, checked) {
        ...
        if (checked) {
            ...
            // Marcar todas las frases como completadas
            if (historia.id) {
    ...
    // ============================================================
    // 🔥 ESTUDIAR HISTORIA COMPLETA - CORREGIDO
    // ============================================================
        ...
        try {
            ...
            if (!historiaId || !this._historiasGuardadas[idx]) {
                ...
                if (historiaId) {
                    ...
                    // Actualizar la historia en el array
    ...
    _inyectarBotonVolverTonos() {
        ...
        // Ocultar el botón de "Libro de Lectura" si existe
    ...
    async _volverAlModoTonos() {
        ...
        try {
            ...
            if (this._historiaEnEstudio !== null && this._historiaEnEstudio !== undefined) {
                ...
                if (historia) {
                    // Verificar si todas las frases están completadas
                    ...
                    if (todasCompletadas && frasesTotales > 0) {
                        ...
                        if (historia.id) {
                            ...
                            if (historiaDB) {
                                ...
                                // Disparar evento de historia completada
            ...
            if (this._core) {
                ...
                setTimeout(() => {
                    ...
                    // Limpiar el índice después de usarlo
                    this._historiaEnEstudio = null;
                }, 300);
            }
    ...
    async _guardarHistoria(idx) {
        ...
        try {
            ...
            if (historiaId) {
                ...
                this._historiasGuardadas[idx] = true;
                this._core?.mostrarToast(`✅ Historia "${historia.titulo}" guardada en Mi Espacio (${frasesGuardadas} frases)`, 'success');
        ...
        try {
            ...
            const template = {
                "_INSTRUCCIONES_PARA_IA": {
                    ...
                    "significado_base": this._tonosData.significado || this._caracterActual,
                ...
                "meta": {
                    ...
                    "version": "7.2",
                    "titulo_historia": `Historia de tonos - ${this._caracterActual}`
                },
    ...
    _mostrarModalHistoriaJSON(template) {
        ...
        infoDiv.innerHTML = `
            ...
            4. Cuando la IA te devuelva el JSON completado, pégalo aquí y pulsa <strong>"Importar"</strong>.<br>
            5. La historia se guardará como una unidad completa con todas sus frases.<br>
            <br>
    ...
    async _importarHistoriaJSON(data) {
        ...
        try {
            ...
            const tituloHistoria = data.meta?.titulo_historia || `Historia de tonos - ${caracterBase}`;
            ...
            const nuevaHistoria = {
                ...
                resumen: data.lineas.slice(0, 3).map(l => l.hanzi).join(' ... '),
                completada: false,
                _guardada: true
            };
            ...
            const mensaje = `✅ Historia importada correctamente\n\n` +
                ...
                `❌ Errores: ${errores}\n\n` +
                `💡 La historia "${tituloHistoria}" está disponible para estudiar.`;
    ...
    async _limpiarHistorias() {
        ...
        this._guardarEstadoCompleto();
        this._renderizarPanel();
        this._core?.mostrarToast('🧹 Historias limpiadas correctamente', 'success');
    }
...
console.log('🎵 UI Estudio de Tonos v7.2 - COMPLETO Y CORREGIDO');
