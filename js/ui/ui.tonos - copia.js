// ============================================================
// UI ESTUDIO DE TONOS v7.7 - MULTIIDIOMA COMPLETO
...
class UITonos {
    constructor() {
        ...
        if (!cargado || this._historiasGeneradas.length === 0) {
            ...
            this._cargarDesdeIndexedDB(idiomaInicial).then(function(resultado) {
                if (resultado) {
                    console.log('✅ Carga inicial desde IndexedDB completada: ' + this._historiasGeneradas.length + ' historias');
    ...
    _guardarEstadoPorIdioma(idioma) {
        ...
        try {
            ...
            var data = {
                ...
                timestamp: Date.now(),
                idioma: idioma,
                historiasGeneradas: this._historiasGeneradas.map(function(h) {
                    return {
                        ...h,
                        frases: h.frases ? h.frases.map(function(f) {
                            return {
                                ...f,
                                palabras: f.palabras || []
                            };
            ...
            console.log('💾 Estado de Tonos guardado en localStorage para idioma: ' + idioma + ' (' + this._historiasGeneradas.length + ' historias)');
    ...
    async _guardarEnIndexedDB(idioma) {
        ...
        try {
            if (typeof db === 'undefined' || !db._initialized) {
                console.warn('⚠️ DB no disponible para guardar Tonos');
                return false;
            }
            ...
            if (guardadoExitoso) {
                console.log('💾 Estado de Tonos guardado en IndexedDB para idioma: ' + idioma + ' (' + this._historiasGeneradas.length + ' historias)');
            }
    ...
    async _cargarDesdeIndexedDB(idioma) {
        ...
        try {
            if (typeof db === 'undefined' || !db._initialized) {
                console.warn('⚠️ DB no disponible para cargar Tonos');
                return false;
            }
            ...
            if (configs && configs.length > 0 && configs[0].valor) {
                ...
                console.log('   📊 ' + (data.historiasGeneradas?.length || 0) + ' historias');
                ...
                console.log('✅ Datos cargados desde IndexedDB: ' + this._historiasGeneradas.length + ' historias');
                return true;
            }
    ...
    _cargarEstadoPorIdioma(idioma) {
        ...
        try {
            ...
            if (stored) {
                ...
                console.log('   📊 ' + (parsed.historiasGeneradas?.length || 0) + ' historias');
    ...
    _aplicarDatos(data) {
        ...
        this._historiasGeneradas = data.historiasGeneradas || [];
        ...
        console.log('✅ Datos aplicados: ' + this._historiasGeneradas.length + ' historias');
    }
    ...
    async init(core) {
        ...
        console.log('🎵 UI Tonos v7.7: Inicializado (MULTIIDIOMA)');
        console.log('   📊 ' + this._historiasGeneradas.length + ' historias para idioma: ' + idiomaActual);
        console.log('   💾 Datos cargados: ' + (this._datosPorIdioma[idiomaActual] ? '✅ Sí' : '❌ No'));
        
        if (this._historiasGeneradas.length > 0) {
            console.log('   📖 Renderizando ' + this._historiasGeneradas.length + ' historias');
        }
    ...
    _renderizarPanelConCaracteres(caracteresRaiz) {
        ...
        html += '<div><h2 style="font-size:22px;font-weight:800;color:var(--dark);margin:0;">🎵 Estudio de Tonos <span style="font-size:14px;font-weight:400;color:var(--gray);margin-left:8px;">' + nombreIdioma + '</span><span style="font-size:11px;font-weight:400;color:var(--success);margin-left:8px;">🔊 Tonal</span><span style="font-size:10px;color:var(--info);margin-left:8px;">💾 ' + this._historiasGeneradas.length + ' historias</span></h2>';
        html += '<p style="font-size:13px;color:var(--gray);margin:4px 0 0;">Nivel <strong>' + nivelActual + '</strong> · ' + caracteresRaiz.length + ' caracteres raíz disponibles <span style="font-size:11px;color:var(--gray-light);margin-left:8px;">📖 ' + this._historiasLeidas.size + ' leídas</span><span style="font-size:11px;color:var(--primary);margin-left:8px;">💾 ' + this._historiasGeneradas.length + ' historias</span><span style="font-size:10px;color:var(--secondary);margin-left:8px;">🌍 ' + idiomaActivo + '</span></p></div>';
    ...
    _renderizarSeleccionCaracter(caracteresFiltrados, caracteresRaiz) {
        ...
        for (var ipi = 0; ipi < itemsPagina.length; ipi++) {
            ...
            html += '<div style="font-size:9px;color:var(--gray-light);margin-top:2px;">' + familia2 + '</div>';
            html += '</div>';
        }
        html += '</div>';
    ...
    _renderizarEstudioTonos() {
        ...
        var historias = this._historiasGeneradas;
        ...
        for (var hi = 0; hi < historias.length; hi++) {
            if (this._historiasLeidas.has('historia_' + hi)) totalLeidas++;
        }
        ...
        if (historias.length > 0) {
            ...
            html += '<div style="display:flex;align-items:center;gap:8px;"><h4 style="font-size:14px;font-weight:600;color:var(--dark);margin:0;">📖 Historias generadas (' + historias.length + ')</h4><span style="font-size:10px;color:var(--success);">✅ ' + totalLeidas + ' leídas</span>' + (totalPaginasHistorias > 1 ? '<span style="font-size:10px;color:var(--gray-light);">· Página ' + this._paginaHistorias + '/' + totalPaginasHistorias + '</span>' : '') + '</div>';
            ...
            for (var hpi = 0; hpi < historiasPagina.length; hpi++) {
                ...
                var esGuardada = this._historiasGuardadas[globalIdx] === true;
                var esLeida = this._historiasLeidas.has('historia_' + globalIdx);
                ...
                var tituloMostrar = historia.titulo || 'Historia de tonos - ' + this._caracterActual;
                
                html += '<div style="background:' + (esLeida ? 'var(--success)04' : 'var(--bg)') + ';border-radius:10px;padding:12px 16px;border:2px solid ' + (completada ? 'var(--success)' : (esLeida ? 'var(--success)' : 'var(--light)')) + ';display:flex;flex-direction:column;gap:6px;cursor:pointer;" onclick="window.UITonos._estudiarHistoriaCompleta(' + globalIdx + ')" onmouseover="this.style.borderColor=\'var(--primary)\'" onmouseout="this.style.borderColor=\'' + (completada ? 'var(--success)' : (esLeida ? 'var(--success)' : 'var(--light)')) + '\'">';
                ...
                html += '<label style="display:flex;align-items:center;gap:3px;font-size:9px;cursor:pointer;padding:2px 8px;background:' + (completada ? 'var(--success)15' : 'var(--bg)') + ';border-radius:10px;border:1px solid ' + (completada ? 'var(--success)' : 'var(--light)') + ';" onclick="event.stopPropagation();"><input type="checkbox" ' + (completada ? 'checked' : '') + ' onchange="window.UITonos._toggleHistoriaCompletada(' + globalIdx + ', this.checked)" style="margin:0;width:12px;height:12px;cursor:pointer;"><span style="color:' + (completada ? 'var(--success)' : 'var(--gray)') + ';font-size:8px;">' + (completada ? '✅' : '⬜') + '</span><span style="font-size:7px;color:var(--gray-light);">Completada</span></label>';
                html += '<label style="display:flex;align-items:center;gap:3px;font-size:9px;cursor:pointer;padding:2px 8px;background:' + (esLeida ? 'var(--success)15' : 'var(--bg)') + ';border-radius:10px;border:1px solid ' + (esLeida ? 'var(--success)' : 'var(--light)') + ';" onclick="event.stopPropagation();"><input type="checkbox" ' + (esLeida ? 'checked' : '') + ' onchange="window.UITonos._toggleHistoriaLeida(' + globalIdx + ', this.checked)" style="margin:0;width:12px;height:12px;cursor:pointer;"><span style="color:' + (esLeida ? 'var(--success)' : 'var(--gray)') + ';font-size:8px;">' + (esLeida ? '✅' : '⬜') + '</span><span style="font-size:7px;color:var(--gray-light);">Leída</span></label>';
                html += '</div>';
                html += '<div style="display:flex;gap:4px;">';
                if (!esGuardada) {
                    html += '<button onclick="event.stopPropagation();window.UITonos._guardarHistoria(' + globalIdx + ')" style="padding:2px 12px;font-size:10px;background:linear-gradient(135deg,#00B894,#55EFC4);color:white;border:none;border-radius:4px;cursor:pointer;"><i class="fas fa-save"></i> Guardar</button>';
                }
                html += '<button onclick="event.stopPropagation();window.UITonos._leerHistoriaCompleta(' + globalIdx + ')" style="padding:2px 12px;font-size:10px;background:var(--secondary);color:white;border:none;border-radius:4px;cursor:pointer;"><i class="fas fa-book"></i> Leer</button>';
                html += '<button onclick="event.stopPropagation();window.UITonos._estudiarHistoriaCompleta(' + globalIdx + ')" style="padding:2px 12px;font-size:10px;background:var(--primary);color:white;border:none;border-radius:4px;cursor:pointer;"><i class="fas fa-play"></i> Estudiar</button>';
                html += '<button onclick="event.stopPropagation();window.UITonos._eliminarHistoria(' + globalIdx + ')" style="padding:2px 12px;font-size:10px;background:var(--danger);color:white;border:none;border-radius:4px;cursor:pointer;" title="Eliminar esta historia"><i class="fas fa-trash"></i></button>';
                html += '</div></div></div>';
            }
    ...
    _irPaginaHistorias(pagina) {
        var totalHistorias = this._historiasGeneradas.length;
    ...
    async _toggleHistoriaCompletada(idx, checked) {
        var historia = this._historiasGeneradas[idx];
        if (!historia) return;
    ...
    async _eliminarHistoria(idx) {
        ...
        try {
            ...
            for (var i = 0; i < this._historiasGeneradas.length; i++) {
                var oldKey = 'historia_' + (i >= idx ? i + 1 : i);
                var newKey = 'historia_' + i;
    ...
    async _importarHistoriaJSON(data) {
        ...
        try {
            ...
            var idx2 = this._historiasGeneradas.length;
            ...
            // this._historiasLeidas.add('historia_' + idx2); // <-- ELIMINADO
            
            // No se guarda automáticamente, el usuario decide si guardar
            // this._historiasGuardadas[idx2] = true; // <-- ELIMINADO
    ...
    async _guardarHistoria(idx) {
        ...
        try {
            var idioma = this._obtenerIdiomaActual() || 'zh';
            var nivel = this._obtenerNivelRealUsuario();
            var caracterBase = this._caracterActual || 'Carácter base';
    ...
    _limpiarHistorias() {
        ...
        for (var i = 0; i < this._historiasGeneradas.length; i++) {
            var historia = this._historiasGeneradas[i];
            if (historia.id) {
                try {
    ...
    async _estudiarHistoriaCompleta(idx) {
        ...
        try {
            ...
            for (var fi2 = 0; fi2 < frasesDB.length; fi2++) {
                var f2 = frasesDB[fi2];
                var historiaOriginal = this._historiasGeneradas[idx];
                var tonoEncontrado = 'Desconocido';
                var caracterBase2 = this._caracterActual || 'Carácter base';
                ...
                if (historiaOriginal && historiaOriginal.frases) {
                    for (var hfi = 0; hfi < historiaOriginal.frases.length; hfi++) {
                        ...
                        if (hf.hanzi === f2.original || hf.original === f2.original) {
                            ...
                            caracterBase2 = hf.caracter_usado || this._caracterActual || 'Carácter base';
                            break;
                        }
                    }
                }
    ...
    async _volverAlModoTonos() {
        ...
        try {
            if (this._historiaEnEstudio !== null && this._historiaEnEstudio !== undefined) {
                var idx = this._historiaEnEstudio;
                var historia = this._historiasGeneradas[idx];
            ...
            if (this._core) {
                ...
                setTimeout(function() {
                    this._renderizarPanel();
                    var idx2 = this._historiaEnEstudio;
                    if (idx2 !== null && idx2 !== undefined && this._historiasGeneradas[idx2]?.completada) {
...
console.log('🎵 UI Estudio de Tonos v7.7 - MULTIIDIOMA COMPLETO');
