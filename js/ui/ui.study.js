...
(function() {
    ...
    class UIStudy {
        ...
        cargar(core) {
            ...
            // 🔥 VERIFICAR SI EL TEMA ESTÁ COMPLETADO ANTES DE CARGAR
            if (pipeline._estudiandoTema && pipeline._temaActual) {
                ...
                if (frases.length > 0) {
                    ...
                    console.log(`📊 Progreso del tema al cargar: ${progreso}%`);
                    
                    if (progreso >= 100) {
                        this._temaFinalizado = true;
                        db.obtenerTema(pipeline._temaActual).then(tema => {
                            if (tema) {
                                this._mostrarPantallaFinalizacion(tema);
                            }
                        });
                        return;
                    }
        ...
        async _desmarcarTemaYReiniciar() {
            ...
            const confirmar = await this.core?.confirm(
                `🔄 ¿Reiniciar el tema "${tema.nombre}"?\n\n` +
                `Esto:\n` +
                `• Pondrá TODAS las frases del tema a RCN 0\n` +
                `• Desmarcará el tema como completado\n` +
                `• Te permitirá estudiarlo de nuevo desde cero\n\n` +
                `¿Continuar?`,
                '🔄 Reiniciar Tema'
            );
            ...
            try {
                ...
                if (temaActual) {
                    temaActual.estado = 'en_curso';
                    temaActual._completado = false;
                    delete temaActual._fechaCompletado;
                    await db.update('temas', temaActual);
                }
            } catch (e) {
                console.warn('⚠️ Error actualizando tema en DB:', e);
            }

            // 4. 🔥 RESETEAR TODAS LAS FRASES DEL TEMA A RCN 0
            const frases = pipeline.frases || [];
            let reseteadas = 0;
            ...
            for (const f of frases) {
                ...
                if (progreso) {
                    // Resetear completamente el progreso
                    progreso.rcn = 0;
                    progreso.fase = 1;
                    progreso.estado = 'en_curso';
                    progreso.repasosExitosos = 0;
                    progreso.repasosFallidos = 0;
                    progreso.intervaloActual = 3600000;
                    progreso.proximoRepaso = Date.now() + 3600000;
            ...
            console.log(`✅ ${reseteadas} frases reseteadas a RCN 0`);

            // 5. Recargar el tema en el pipeline
            await pipeline.estudiarTema(tema.id);

            // 6. Resetear estado de UI
            this._temaFinalizado = false;
            this._modoVista = 'frase';
            this._libroAbierto = false;
            this._progresoMostrado = 0;
            ...
            this.core?.mostrarToast(`✅ Tema "${tema.nombre}" reiniciado (${reseteadas} frases reseteadas)`, 'success');

            // 8. Navegar al estudio
            if (this.core) {
                this.core.irAModulo('study');
            }

            // 9. Renderizar la primera frase
            setTimeout(() => {
                this._renderizarFraseInteractiva();
            }, 300);

            // 10. Actualizar la vista de temas
            if (window.UITemas) {
                setTimeout(async () => {
        ...
        // ============================================================
        // 🔥 MOSTRAR MENSAJE DE TEMA COMPLETADO
        // ============================================================

        _mostrarMensajeTemaCompletado() {
            this.core?.mostrarToast('✅ Este tema ya está completado al 100%. Ve a "Temas" para verlo.', 'info');
            
            this.core?.alert(
                '📚 **Tema Completado**\n\n' +
                'Este tema ya ha sido completado al 100%.\n\n' +
                '✅ Puedes verlo marcado en verde en la lista de Temas.\n' +
                '🔄 Si quieres reestudiar, usa el botón "Desmarcar" en el detalle del tema.\n' +
                '🔒 También puedes ocultar los temas completados desde el botón "Ocultar completados".',
                '✅ Tema Completado'
            );
        }
