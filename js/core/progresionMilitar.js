...
(function (global) {
    ...
    class ProgresionMilitar {
        constructor() {
            ...
            window.addEventListener('formacionPasoCompletado', () => window.UIDashboard?._cargarDashboardInicial?.());
        }
        ...
        registrarPasoFormacion(paso) {
            const campos = { 1: 'manualLeido', 2: 'importacionProbada', 3: 'ondaProbada', 4: 'estudioProbado' };
            const campo = campos[Number(paso)];
            if (!campo || this.estado[campo] === true) return;
            this.estado[campo] = true;
            this._guardar();
            window.dispatchEvent(new CustomEvent('formacionPasoCompletado', {
                detail: { paso: Number(paso), campo, fecha: Date.now() }
            }));
        }
        ...
        abrirPanelSeguro() {
            return this.abrirPanel().catch(async error => {
                ...
                try {
                    ...
                    const hitosHtml = misiones.map(m => `<div style="padding:9px 11px;border-radius:9px;background:${m.hecho ? 'var(--success)08' : 'var(--bg)'};color:${m.hecho ? 'var(--success)' : 'var(--gray)'};">${m.hecho ? '✅' : '⬜'} ${t(m.texto)}${!m.hecho ? `<button type="button" onclick="window.ProgresionMilitar.abrirPasoFormacion(${m.paso})" style="float:right;border:0;background:transparent;color:var(--primary);font-weight:700;cursor:pointer;">→ ${t('Abrir')}</button>` : ''}<div style="font-size:11px;margin-top:3px;">${escape(t(m.detalle))}</div></div>`).join('');
